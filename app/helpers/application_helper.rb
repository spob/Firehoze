...
module ApplicationHelper
  ...
  def std_formatted_address address1, address2, city, state, postal_code, country, newline_character = "\n"
    "#{h(address1)}#{h(newline_character)}#{h(address2) + h(newline_character) unless (address2.nil? or address2.empty?)}#{city}, #{state} #{postal_code}#{newline_character}#{I18n.t(country, :scope => 'countries')}"
  end
